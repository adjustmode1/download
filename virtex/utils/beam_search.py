...
class AutoRegressiveBeamSearch:
    ...
    def search(
        ...
        textual: any
            ...
            target = torch.tensor([[self.sos_index]]) # BOS bằng mấy
            with torch.no_grad():
                output = textual.decode(target.to(self.device), memory)
                output = torch.stack([ textual.generator(out[0, -1, :]) for out in output ])
            ...
            while keep_generating:
                ...
                with torch.no_grad():
                    ...
                    logits = torch.mean(torch.stack([ textual.generator(out[:, -1, :]) for out in output ]), dim=0)
