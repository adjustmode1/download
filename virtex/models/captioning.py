...
class CaptioningModel(nn.Module):
    ...
    def forward(self, batch: Dict[str, torch.Tensor]) -> Dict[str, Any]:
        ...
        if "caption_tokens" in batch:
            ...
            logits = [self.textual.generator(out) for out in output ] 
            logits = [ torch.flatten(prb, start_dim=0, end_dim=1) for prb in logits ]
            tgt_output = torch.flatten(tgt_output) # chuyển sang 1 chiều
            loss = [ self.loss(prb, tgt_output.to(self.device)) for prb in logits ]
            loss = sum(loss)
            ...
            if self.caption_backward:
                ...
                logits = [self.backward_textual.generator(out) for out in output ] 
                logits = [ torch.flatten(prb, start_dim=0, end_dim=1) for prb in logits ]
                backward_tgt_output = torch.flatten(backward_tgt_output) # chuyển sang 1 chiều
                backward_loss = [ self.loss(prb, backward_tgt_output.to(self.device)) for prb in logits ]
