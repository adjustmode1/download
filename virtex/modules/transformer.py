...
class PositionalEncoding(nn.Module):
    def __init__(self, seq_length, in_dim, drop_val=0.1):
        ...
        pos = np.arange(0, seq_length)[:, None] # chuyển sang cột vd shape(10,)=>shape(10,1)
...
class TransformerEncoder(nn.Module):
    def __init__(self, nb_layers, in_dim, ff_dim, nb_heads, drop_val=0.1, pre_norm=False):
        ...
        self.encoders = timm.create_model('vit_base_patch16_224',pretrained=True,num_classes=0)
        self.encoders.eval()
    ...
    def forward(self, src, mask=None, key_padding_mask=None):
        B = src.shape[0]
        src = self.encoders.patch_embed(src)

        cls_tokens = self.encoders.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        src = th.cat((cls_tokens, src), dim=1)
        src = src + self.encoders.pos_embed
        src = self.encoders.pos_drop(src)
        attn = None

        for blk in self.encoders.blocks:
            src = blk(src)

        src = self.encoders.norm(src)
        return src  # ignore src 
...
class Transformer(nn.Module):    
    def __init__(self, in_dim, ff_dim, nb_heads, encoder_depth, decoder_depth, drop_val=0.1, pre_norm=False):
        ...
        self.encoder = TransformerEncoder(
            ...
            in_dim=in_dim, # in_dim là hd_dim của captiontransformer
            ff_dim=ff_dim,
            nb_heads=nb_heads,
            drop_val=drop_val,
            pre_norm=pre_norm 
